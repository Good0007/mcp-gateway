...
2.  **浏览插件**：查看推荐的 MCP 插件列表，包括官方和社区贡献的插件。
3.  **安装插件**：点击插件卡片上的 **"Install"** 按钮，系统会自动将其添加到服务列表中。

### 第三步：管理服务 (Service Management)

1.  **进入服务管理**：点击顶部导航栏的 **"服务管理" (Services)**。
2.  **添加自定义服务**：点击右上角的 **"+ 添加服务"** 按钮，支持配置 stdio 命令或 SSE/HTTP URL。
3.  **导入配置**：点击 **"导入配置"** 按钮，支持导入 Claude Desktop / VS Code 的 `config.json` 配置文件。
4.  **服务控制**：
    *   **启动/停止**：点击服务卡片上的运行状态图标控制服务启停。
    *   **编辑配置**：点击编辑图标（铅笔）修改服务参数、环境变量等。
    *   **删除服务**：点击删除图标（垃圾桶）移除服务。

### 第四步：连接管理 (Connection Management)

在此页面配置 Mcp Gateway 与外部系统的连接。

1.  **进入连接管理**：点击顶部导航栏的 **"连接管理" (Connection)**。
2.  **小智 (XiaoZhi) 接入**：
    *   **Endpoint**: 输入小智服务端的 WebSocket 地址。
    *   **Token**: 输入认证 Token。
    *   **Enabled Services**: 勾选希望暴露给小智使用的服务（例如 `filesystem`, `fetch`）。
    *   **保存连接**：点击保存后，Gateway 会自动尝试连接小智。
3.  **MCP 聚合服务 (MCP Proxy)**：
    *   **Enabled Services**: 勾选希望通过 SSE 接口暴露给 Trae/VSCode 等客户端使用的服务。
